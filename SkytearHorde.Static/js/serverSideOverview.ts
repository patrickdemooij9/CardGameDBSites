export default (config: Config) => ({
  ...
  registerFilter(element: HTMLInputElement, name: string) {
    var filter: Filter = this.filters.find(
  ...
  updateFilterByValues(name: string, value: string, option: ItemOption) {
    const filter: Filter = this.filters.find(
      (item: Filter) => item.key === name
    );
    if (!filter) {
      return;
    }
  ...
  updateOverview() {
    ...
    const elementLessFilters = this.filters.filter(
      (item: Filter) => item.elementLess
    ) as Filter[];
    elementLessFilters.forEach((filter) => {
      const items = filter.filterItems.filter(
        (item) => item.option === ItemOption.Include
      );
  ...
  updateUrl() {
    ...
    this.filters.forEach((filter: Filter) => {
      const checkedItems = filter.filterItems.filter(
        (item) => item.option === ItemOption.Include
      );
