...
<template>
    <nav :class="{ 'md:text-white': content.textColorIsWhite, 'md:text-black': !content.textColorIsWhite, 'active': isOpen }"
        ...
        <div class="container md:px-8">
            <div class="flex justify-between overflow-auto">
                <NuxtLink href="/" class="ml-2 no-underline">
                    <img :src="content.navigationLogoUrl" class="h-14 py-2 pr-8" />
                </NuxtLink>

                <div :class="{ 'fixed top-14 bg-slate-50 h-full w-full z-20 flex-col': isOpen, 'hidden': !isOpen || content.createDeckMode, }"
                    class="grow md:justify-between md:flex">
                    <div class="flex flex-col mt-4 mx-2 md:gap-8 md:h-full md:flex-row md:mt-0">
                        <div v-for="item in content.items" :class="{ 'group': item.children.length > 0 }">
                            <p class="flex items-center hover:text-slate-400 md:h-full gap-2 py-3 md:py-0">
                                <NuxtLink :href="item.url" class="no-underline">
                                    {{ item.name }}
                                </NuxtLink>
                            ...
                            <div v-if="item.children.length > 0"
                                ...
                                <NuxtLink v-for="child in item.children" :href="child.url"
                ...
                <div @click="isOpen = !isOpen" :class="{ 'hidden': content.createDeckMode }"
