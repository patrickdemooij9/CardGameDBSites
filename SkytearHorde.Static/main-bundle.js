...
exports["default"] = (function (initModel) { return ({
    model: initModel,
    expansion: '',
    expansionDropdownOpen: false,
    faction: '',
    factionDropdownOpen: false,
    hasAbility: function (card, abilityType) {
        return card.abilities.some(function (ability) { return ability.type === abilityType; });
    },
    getAbility: function (card, abilityType) {
        return card.abilities.find(function (ability) { return ability.type === abilityType; });
    },
    getAllAbilities: function (abilityType) {
        var _this = this;
        return this.model.cards.filter(function (card) { return _this.hasAbility(card, abilityType); }).map(function (card) { return _this.getAbility(card, abilityType); });
    },
    toggleExpansionDropdown: function () {
        this.expansionDropdownOpen = !this.expansionDropdownOpen;
    },
    togglefactionDropdown: function () {
        this.factionDropdownOpen = !this.factionDropdownOpen;
    },
    getFilteredCards: function () {
        var _this = this;
        return this.model.cards.filter(function (item) {
            var valid = true;
            if (_this.expansion != '' && _this.getAbility(item, 'expansion') !== _this.expansion) {
                valid = false;
            }
            if (_this.faction != '' && _this.getAbility(item, 'faction') !== _this.faction) {
                valid = false;
            }
            return valid;
        });
...
exports["default"] = (function (initModel) { return ({
    ...
    selectedCards: initModel.initialCards.map(function (item) { return ({
        card: item,
    ...
    addCard: function (id) {
        ...
        if (card) {
            var existedCard = this.selectedCards.find(function (item) { return item.card.id === id; });
            if (existedCard) {
                existedCard.amount++;
            }
            else {
    ...
    removeCard: function (id) {
        var cardToRemove = this.selectedCards.find(function (item) { return item.card.id === id; });
        if (cardToRemove) {
            if (cardToRemove.amount == 1) {
                var index = this.selectedCards.indexOf(cardToRemove);
    ...
    getSelectedCards: function (group) {
        return this.selectedCards.filter(function (item) { return item.card.group.toLocaleLowerCase() === group.toLocaleLowerCase(); });
    },
    isEmpty: function (group) {
        return this.getSelectedCards(group).length === 0;
    },
    maxReached: function (card) {
        var selectedCard = this.selectedCards.find(function (item) { return item.card.id === card.id; });
        if (!selectedCard) {
            return false;
        }
        return selectedCard.amount >= card.maxAmount;
    },
    cardCountOfRarity: function (rarity) {
        var items = this.selectedCards.filter(function (item) { return item.card.rarity === rarity; }).map(function (item) { return item.amount; });
        if (items.length === 0) {
            return 0;
        }
        return items.reduce(function (partialSum, item) { return partialSum + item; });
    },
    currentDeckAmount: function (excludeCastle) {
        return this.selectedCards.map(function (item) { return item.amount; }).reduce(function (partialSum, item) { return partialSum + item; }) - (excludeCastle ? 1 : 0);
    },
    validDeckAmount: function () {
        var currentAmount = this.currentDeckAmount(false);
        return currentAmount === this.cardAmount;
    },
    validLegendaryAmount: function () {
        return this.cardCountOfRarity(Rarity.Legendary) <= 2;
    },
    validMythicAmount: function () {
        return this.cardCountOfRarity(Rarity.Mythic) <= 6;
    },
    isValidForSave: function () {
        return this.validLegendaryAmount() && this.validMythicAmount();
    },
    isValidDeck: function () {
    ...
    submitForm: function () {
        this.dataString = JSON.stringify({
            ...
            cards: this.selectedCards.map(function (item) { return ({
                cardId: item.card.id,
                amount: item.amount
            }); })
        });
    ...
    getMainCard: function () {
        return this.selectedCards.find(function (item) { return item.card.group === "Castle"; }).card;
    },
    toggleMarkdownPreview: function () {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!!this.markdownPreview) return [3 /*break*/, 3];
                        return [4 /*yield*/, fetch("/umbraco/api/markdown/preview", {
                                method: 'POST',
                                body: JSON.stringify({
...
exports["default"] = (function (initModel) { return ({
    ...
    addToSquad: function (squad, character) {
        var characterSlot = this.getCharacterSpotValidForSquad(squad, character);
        if (characterSlot) {
            characterSlot.id = character.id;
        }
    },
    removeCharacterFromSquad: function (character) {
        this.removeFromSquad(this.getSlotByCharacter(character));
    },
    removeFromSquad: function (characterSlot) {
        characterSlot.id = undefined;
    },
    selectCharacter: function (character) {
        this.selectedCharacter = character;
        this.selectedCharacterImage = character.images.find(function (item) { return item.isPrimaryImage; });
    },
    selectImage: function (image) {
        this.selectedCharacterImage = image;
    },
    getCharacterSpotValidForSquad: function (squad, character) {
        var _this = this;
        return squad.slots.find(function (slot) {
            if (!slot.id && _this.isAllowedForSquad(slot, character)) {
                return true;
            }
        });
    },
    isAllowedForSquad: function (slot, character) {
        var valid = true;
        slot.requirements.forEach(function (element) {
            var characterAbility = character.abilities.find(function (item) { return item.type === element.type; });
            if (!characterAbility || characterAbility.value !== element.value) {
                valid = false;
            }
        });
        return valid;
    },
    getSlotByCharacter: function (character) {
        var slot = undefined;
        this.squads.forEach(function (squad) {
            var foundSlot = squad.slots.find(function (slot) { return slot.id === character.id; });
            if (foundSlot) {
                slot = foundSlot;
            }
        });
    ...
    isPresentInSquad: function (character) {
        return this.getSlotByCharacter(character) !== undefined;
    },
    getCharacterById: function (id) {
        return this.characters.find(function (item) { return item.id === id; });
    },
    getAbilityValueByType: function (character, type) {
        return character.abilities.find(function (item) { return item.type === type; }).value;
    },
    hasMainImage: function (character) {
        return character.images.some(function (item) { return item.isPrimaryImage; });
    },
    getMainImageUrl: function (character) {
        return character.images.find(function (item) { return item.isPrimaryImage; }).imageUrl;
    },
    isValidForSave: function () {
        return true;
    },
...
var createDeck_1 = __webpack_require__(/*! ./createDeck */ "./js/createDeck.ts");
var cardOverview_1 = __webpack_require__(/*! ./cardOverview */ "./js/cardOverview.ts");
var createSquad_1 = __webpack_require__(/*! ./createSquad */ "./js/createSquad.ts");
window.Alpine = alpinejs_1["default"];
alpinejs_1["default"].data('createDeck', createDeck_1["default"]);
alpinejs_1["default"].data('cardOverview', cardOverview_1["default"]);
alpinejs_1["default"].data('createSquad', createSquad_1["default"]);
