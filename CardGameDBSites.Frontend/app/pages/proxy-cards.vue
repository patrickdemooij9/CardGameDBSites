...
function addCard(card: CardDetailApiModel) {
  const existing = selectedCards.value.find((c) => c.card.baseId === card.baseId);
  if (existing) {
    existing.amount = Math.min(existing.amount + 1, 99);
  } else {
...
function removeCard(baseId: number) {
  selectedCards.value = selectedCards.value.filter((c) => c.card.baseId !== baseId);
}

function increaseAmount(baseId: number) {
  const entry = selectedCards.value.find((c) => c.card.baseId === baseId);
  if (entry) entry.amount = Math.min(entry.amount + 1, 99);
}
...
<template>
  <div class="container px-4 py-8 md:px-8">
    <h1 class="text-2xl font-bold mb-2">Proxy Cards</h1>
    <p class="text-gray-600 mb-6">
      Search for cards, set the amount you need, and download a printable proxy
      sheet.
    </p>
    ...
    <div class="bg-white rounded p-4">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold">
          Selected Cards
          <span v-if="selectedCards.length > 0" class="text-gray-500 text-sm font-normal ml-2">
            ({{ selectedCards.length }} {{ selectedCards.length === 1 ? "card" : "cards" }}, {{ totalCards }} {{ totalCards === 1 ? "copy" : "copies" }})
          </span>
        </h2>
        <button
      ...
      <div v-if="selectedCards.length === 0" class="text-gray-400 text-center py-8">
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        >
          ...
          <p class="text-xs text-center leading-tight">{{ entry.card.displayName }}</p>
          ...
          <div class="flex items-center gap-1">
            ...
            <span class="w-6 text-center text-sm font-semibold">{{ entry.amount }}</span>
            <button
              class="border rounded px-1.5 py-0.5 hover:bg-gray-100"
